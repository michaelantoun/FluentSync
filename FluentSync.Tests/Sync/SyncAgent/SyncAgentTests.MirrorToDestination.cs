using FluentSync.Sync;
using FluentSync.Sync.Configurations;

namespace FluentSync.Tests.Sync.SyncAgent
{
    public partial class SyncAgentTests
    {
        [Fact]
        public async Task Sync_MirrorToDestination_Int_NonEmptyLists()
        {
            List<int> source = new List<int> { 5, 4, 9 }
                , destination = new List<int> { 6, 10, 5 };

            await SyncAgent<int>.Create()
                .Configure((c) => c.SyncMode.SyncModePreset = SyncModePreset.MirrorToDestination)
                .SetComparerAgent(ComparerAgent<int>.Create())
                .SetSourceProvider(source)
                .SetDestinationProvider(destination)
                .SyncAsync(CancellationToken.None);

            source.ShouldBeEquivalentToIgnoringOrder(new List<int> { 5, 4, 9 });
            destination.ShouldBeEquivalentToIgnoringOrder(new List<int> { 5, 4, 9 });
        }

        [Fact]
        public async Task Sync_MirrorToDestination_Int_WithEmptyDestination()
        {
            List<int> source = new List<int> { 5, 4, 9 }
                , destination = new List<int>();

            await SyncAgent<int>.Create()
                .Configure((c) => c.SyncMode.SyncModePreset = SyncModePreset.MirrorToDestination)
                .SetComparerAgent(ComparerAgent<int>.Create())
                .SetSourceProvider(source)
                .SetDestinationProvider(destination)
                .SyncAsync(CancellationToken.None);

            source.ShouldBeEquivalentToIgnoringOrder(new List<int> { 5, 4, 9 });
            destination.ShouldBeEquivalentToIgnoringOrder(new List<int> { 5, 4, 9 });
        }

        [Fact]
        public async Task Sync_MirrorToDestination_Int_WithEmptySourceList()
        {
            List<int> source = new List<int>()
                , destination = new List<int> { 6, 10, 5 };

            await SyncAgent<int>.Create()
                .Configure((c) => c.SyncMode.SyncModePreset = SyncModePreset.MirrorToDestination)
                .SetComparerAgent(ComparerAgent<int>.Create())
                .SetSourceProvider(source)
                .SetDestinationProvider(destination)
                .SyncAsync(CancellationToken.None);

            source.ShouldBeEmpty();
            destination.ShouldBeEmpty();
        }

        [Fact]
        public async Task Sync_MirrorToDestination_Int_WithEmptyLists()
        {
            List<int> source = new List<int>()
                , destination = new List<int>();

            await SyncAgent<int>.Create()
                .Configure((c) => c.SyncMode.SyncModePreset = SyncModePreset.MirrorToDestination)
                .SetComparerAgent(ComparerAgent<int>.Create())
                .SetSourceProvider(source)
                .SetDestinationProvider(destination)
                .SyncAsync(CancellationToken.None);

            source.ShouldBeEmpty();
            destination.ShouldBeEmpty();
        }
    }
}
